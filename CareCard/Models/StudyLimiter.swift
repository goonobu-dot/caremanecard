...
/// 無料プランの制限:
/// 無料=科目①介護支援分野の最初のトピック（約40枚）＋1日20枚まで。
/// 純関数として切り出し、日付境界やカウントのバグを単体テストで固定する。
public enum StudyLimiter {

    public static let freeDailyCardLimit = 20
    /// 無料開放するトピック（介護支援分野の最初のトピック）
    public static let freeUnlockedTopics: Set<String> = ["介護保険制度のしくみ"]

    /// カードが無料ユーザーに閲覧可能かどうか。
    public static func isCardFree(subject: Subject, topic: String) -> Bool {
        subject == .shien && freeUnlockedTopics.contains(topic)
    }
